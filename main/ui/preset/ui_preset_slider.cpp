...
Handle create(lv_obj_t* parent, Ui::UiThemeStyles& styles, const Props& p, const Callbacks& cb) {
    ...
    // Eventos (VALUE_CHANGED + RELEASED) y mantenimiento de UI
    if (cb.onChange || cb.onRelease || p.step > 0 || p.showValue) {
        // user data por instancia; se liberará en LV_EVENT_DELETE
        struct User {
            Handle    h;
            const char* suffix;
            int32_t   step;
        ...
        *u = User{ h, p.suffix, p.step, cb };
        ...
        lv_obj_add_event_cb(h.slider, [](lv_event_t* e){
            ...
            if (code == LV_EVENT_DELETE) {
                // Liberar user data asociada a este slider
                lv_mem_free(u);
                return;
            }
...
void setValue(Handle& h, int32_t v) {
    ...
    if (h.valueL) update_value_label(h.valueL, v, nullptr); // Nota: el sufijo inicial no se conserva aquí
}
...
void setStep(Handle& h, int32_t step) {
    // MVP: el step se fija en create() a través de la user data del callback.
    // Si necesitas cambiar step en runtime, podemos ampliar el Handle o re-vincular el event_cb.
    (void)h; (void)step;
}
