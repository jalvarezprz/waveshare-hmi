 ...
 * @brief Tokens de diseño (header-only): paleta, espaciados, radios, tipografía y helpers LVGL.
 * @defgroup ui_theme_tokens Tokens de UI
 * @ingroup ui_theme
 * @{
 *
 ...
 * - Paneles & Cards
 * - Lista (medidas, estados, divisores)
 * - Botones (tamaños/radio/icon gap, colores por estado)
 * - Tipografía (fuentes reales y fallback)
 *
 * Los tokens son el “source of truth”: estilos y componentes deben leer de aquí.
 */
...
/** @brief Convierte RGBA 0xRRGGBBAA a lv_color_t (RGB). */
static inline lv_color_t rgba_to_lv(std::uint32_t rgba) {
    return lv_color_hex(rgba & 0x00FFFFFFu);
}

/** @name Paleta base y completa (RGBA 0xRRGGBBAA) */
/** @{ */
namespace Color {
    // Superficies
...
/** @name Helpers LVGL de color (derivados de Color::*) */
/** @{ */
...
/** @name Interacción lista (opacidades/outline) */
/** @{ */
namespace Interaction {
    ...
    inline constexpr int      FocusOutlineW  = 2;          ///< ancho outline
    inline constexpr int      FocusOutlinePad= 2;          ///< padding outline
}
...
/** @name Colores de botón por estado */
/** @{ */
...
/** @name Colores de etiqueta */
/** @{ */
inline lv_color_t label_primary_color()   { return color_on_surface(); }
inline lv_color_t label_secondary_color() { return color_muted(); }
/** @} */

/** @name Énfasis de texto (opacidades orientativas) */
/** @{ */
namespace TextEmphasis {
    inline constexpr lv_opa_t High     = LV_OPA_COVER;
    inline constexpr lv_opa_t Medium   = LV_OPA_70;
    inline constexpr lv_opa_t Disabled = LV_OPA_40;
}
/** @} */

/** @name Tamaños tipográficos (guía, px) */
/** @{ */
namespace TypographyPx {
    inline constexpr int Title    = 14;
    inline constexpr int Body     = 12;
    inline constexpr int Caption  = 8;
    inline constexpr int Subtitle = 13;
    inline constexpr int Overline = 10;
}
/** @} */

/** @name Fuentes compiladas (Montserrat *_lat1 si disponible) */
/** @{ */
...
#endif
inline const lv_font_t* font_text() { return font_body(); } ///< sinónimo usado por botones
